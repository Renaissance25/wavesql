...
from ratsql.models.sparc.sparc_infer_tree_traversal import InferenceTreeTraversal
from ratsql.models.sparc.sparc_train_tree_traversal import TrainTreeTraversal
...
def get_field_presence_info(ast_wrapper, node, field_infos):
    ...
    for field_info in field_infos:
        ...
        is_present = field_value is not None and field_value != []      ##查看子节点是否存在

        maybe_missing = field_info.opt or field_info.seq
        is_builtin_type = field_info.type in ast_wrapper.primitive_types   #是否是叶子节点
...
class NL2CodeDecoderPreproc(abstract_preproc.AbstractPreproc):
    ...
    def validate_item(self, item, section):
        parsed_sqls = []
        for index in range(len(item.sqls)):
            parsed = self.grammar.parse(item.sqls[index], section)
            if parsed:
                try:
                    self.ast_wrapper.verify_ast(parsed)
                except AssertionError:
                    return section != 'train', None
                parsed_sqls.append(parsed)
            else:
                return section != 'train', None
        return True, parsed_sqls

    def add_item(self, item, section, validation_info):
        interaction_items = []
        for index, root in enumerate(validation_info):
            if section == 'train':
            ...
            interaction_items.append(
                NL2CodeDecoderPreprocItem(
                    tree=root,
                    orig_code=item.sqls[index]))

        self.items[section].append(interaction_items)
    ...
    def save(self):
        ...
        for section, interaction_items in self.items.items():
            with open(os.path.join(self.data_dir, section + '.jsonl'), 'w') as f:
                for interaction_item in interaction_items:
                    f.write("new interaction\n")
                    for node in interaction_item:
                        f.write(json.dumps(attr.asdict(node)) + '\n')
    ...
    def dataset(self, section):
        ...
        for line in open(os.path.join(self.data_dir, section + '.jsonl')):
            if line.strip() == "new interaction":
                if interaction_item:
                    interaction_items.append(interaction_item)
                interaction_item = []
                continue
            interaction_item.append(NL2CodeDecoderPreprocItem(**json.loads(line)))
    ...
    def _record_productions(self, tree):
        ...
        while queue:
            ...
            node_type = node['_type']           #该节点的type

            # Rules of the form:
            # expr -> Attribute | Await | BinOp | BoolOp | ...
            # expr_seq_elem -> Attribute | Await | ... | Template1 | Template2 | ...
            for type_name in [node_type] + node.get('_extra_types', []):        #如果有_extra_types， 加入到type中
                if type_name in self.ast_wrapper.constructors:
                    ...
                    if is_seq_elem and self.use_seq_elem_rules:
                        self.sum_type_constructors[sum_type_name + '_seq_elem'].add(type_name) #如果是is_seq_elem，则加上'_seq_elem'
                    else:
                        self.sum_type_constructors[sum_type_name].add(type_name)
#"sum_type_constructors": {    "agg_type": ["Avg","Count","Max","Min","NoneAggOp","Sum"],
#sum type 指的是任选其一
            ...
            assert node_type in self.ast_wrapper.singular_types         #根据parse结果，所有节点都应为singular_type
...
#field_presence_infos": {"And": [[true,true]], "Asc": [[]], "Avg": [[]], "Between": [[true,true,true]], "ColUnit": [[true]],
#对应singular_types中field是否为必要，如果有多种选择，则为不同的list；另外，如果为builtin type，则选择builtin type
            ...
            for field_info in self.ast_wrapper.singular_types[node_type].fields:
                ...
                if field_info.seq:
                    ...
                    #例如aggs 包含多个agg， field_value=[agg,agg,agg]
                else:
                    to_enqueue = [field_value]
                for child in to_enqueue:
                    if isinstance(child, collections.abc.Mapping) and '_type' in child: #如果还有子节点
                        queue.append((child, field_info.seq))
                    else:
                        self.primitive_types.add(type(child).__name__)  #加入primitive type

    def _calculate_rules(self):
        offset = 0

        all_rules = []   ##对应的规则，把sum_type, singular_types, primitive_types 对应的规则列出
        rules_mask = {}  ###指的是该节点对应那几条规则！！
...
@registry.register('decoder', 'sparc')
class NL2CodeDecoder(torch.nn.Module):
    ...
    def __init__(
            ...
            enc_recurrent_size=256,     #1024 bert_size 768
            recurrent_size=256,         #512
            dropout=0.,
            desc_attn='bahdanau',       #mha
            copy_pointer=None,
            multi_loss_type='logsumexp',
            sup_att=None,
            use_align_mat=False,        #True
            use_align_loss=False,       #True
    ...
    def compute_mle_loss(self, enc_input, example, desc_enc, final_utterance_state, debug=False):
        ...
        queue = [
            TreeState(
                node=example.tree,
                parent_field_type=self.preproc.grammar.root_type,   ##这个参数指的是当前节点在语法结构中的类型
            )
        ]
        while queue:
            item = queue.pop()
            ...
            if isinstance(node, (list, tuple)):     #如果节点是list 或者 tuple ,例如"agg", "col_unit"之类
                node_type = parent_field_type + '*'     #   'agg*'
                rule = (node_type, len(node))
                rule_idx = self.rules_index[rule]       #用的哪条规则
            ...
            if parent_field_type in self.preproc.grammar.pointers:  #如果预测的是 column,table节点
            ...
            if parent_field_type in self.ast_wrapper.primitive_types:   #如果预测的是 primitive_type
            ...
            type_info = self.ast_wrapper.singular_types[node['_type']]          #该节点类型

            if parent_field_type in self.preproc.sum_type_constructors:         #如果该节点是可选类型的
            ...
            if type_info.fields:
                ...
                rule_idx = self.rules_index[rule]                               #按照记录规则方法，找出当前那条规则
            ...
            for field_info in reversed(type_info.fields):               #reversed以后，因为queue是个栈，所以DFS是left to right
    ...
    def apply_rule(
        ...
        # self.prev_action_emb,         torch [1, 128]
