        ...
        for col_id, column in enumerate(schema.columns):
            ...
            if num_flag:
                if column.type in ["number", "time"]:  # TODO fine-grained date
                    num_date_match[f"{q_id},{col_id}"] = column.type.upper()        #如果question单词 和column类别相同进行连线
            else:
                ret = db_word_match(word, column.orig_name, column.table.orig_name, schema.connection)
                if ret:
                    # print(word, ret)
                    cell_match[f"{q_id},{col_id}"] = CELL_MATCH_FLAG        #如果column中的值 有与 question 相匹配的，则连线。
