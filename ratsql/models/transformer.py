...
def relative_attention_logits(query, key, relation):
    ...
    # key.transpose: [batch, heads, depth, num kvs]
...
#hidden_size = 768,  len(self.relation_ids) =  50 , dropout)
    ...
    def forward(self, query, key, value, relation, mask=None):
        relation_k = self.relation_k_emb(relation)      # enc_lenth, column_num, 768
        relation_v = self.relation_v_emb(relation)      # enc_lenth, column_num, 768
...
class MultiHeadedAttentionWithRelations(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
# h = num_heads = 8, d_model = hidden_size = 768, dropout,
        "Take in model size and number of heads."
        super(MultiHeadedAttentionWithRelations, self).__init__()
        assert d_model % h == 0
        # We assume d_v always equals d_k
        self.d_k = d_model // h         #整数除= int(d_model / h)
        self.h = h
        self.linears = clones(lambda: nn.Linear(d_model, d_model), 4)
        self.attn = None
        self.dropout = nn.Dropout(p=dropout)
    ...
    def forward(self, query, key, value, relation_k, relation_v, mask=None):
        ...
        query, key, value = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.linears, (query, key, value))]
        #query torch.Size([batch, h, num enc, d_k])
...
class Encoder(nn.Module):
    ...
    def __init__(self, layer, layer_size, N, tie_layers=False):
        ...
        if tie_layers:
            ...
            self.layers = clones(layer, N)   #N个transformer.EncoderLayer层
...
#size = hidden_size , self_attn = transformer.MultiHeadedAttentionWithRelations(num_heads, hidden_size, dropout)
# feed_forward = transformer.PositionwiseFeedForward(hidden_size, ff_size=hidden_szie * 4, dropout), num_relation_kinds = len(relation_ids)边的种类
        ...
        self.relation_k_emb = nn.Embedding(num_relation_kinds, self.self_attn.d_k)  #embedding层就是把相应数字转化为后面维度的向量，无论n维向量
...
# d_model = hidden_size = 768, d_ff = hidden_size = 768*4, dropout,
