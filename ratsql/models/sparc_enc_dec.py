...
class EncDecModel(torch.nn.Module):
    ...
    def _compute_loss_unbatched(self, batch, debug=False):
        ...
        for enc_inputs, dec_outputs in batch:
            ...
            for enc_input, dec_output in zip(enc_inputs, dec_outputs):
                q_enc, enc_state = self.encoder(enc_input, pre_enc)
                pre_enc = torch.cat([pre_enc, q_enc], 0)
                loss = self.decoder.compute_loss(enc_input, dec_output, enc_state, debug)
                losses.append(loss)
        if debug:
            return losses
    ...
    def begin_inference(self, orig_item, enc_input, pre_enc):     #inference
        q_enc, enc_state = self.encoder(enc_input, pre_enc)
        self.decoder.begin_inference(enc_state, orig_item)

        return q_enc, self.decoder.begin_inference(enc_state, orig_item)
