...
class SpiderEncoderBertPreproc(SpiderEncoderV2Preproc):
    ...
    def preprocess_item(self, item, validation_info):
        question = self._tokenize(item.text, item.orig['question'])   ## bert分词，把question转化为bert字典能识别的东西
        preproc_schema = self._preprocess_schema(item.schema)
        if self.compute_sc_link:
...
class SpiderEncoderBert(torch.nn.Module):
    ...
    def __init__(
        ...
        update_modules = {
            'relational_transformer':
                spider_enc_modules.RelationalTransformerUpdate, #走这个
            'none':
                spider_enc_modules.NoOpUpdate,
        }
    ...
    def forward(self, descs):
        ...
        for batch_idx, desc in enumerate(descs):
            qs = self.pad_single_sentence_for_bert(desc['question'], cls=True)  #加入 cls和sep
            ...
            token_list = qs + [c for col in cols for c in col] + \
                         [t for tab in tabs for t in tab]        #拼接
            ...
            batch_id_map[batch_idx] = len(batch_id_map)     #把长度超过512的去掉后，进行数据的记录
        ...
        assert len(long_seq_set) == 0  # remove them for now 这不是表明已经空了吗？？？？
        ...
        for batch_idx, desc in enumerate(descs):
            ...
            if batch_idx in long_seq_set: ##前面已经为0了，这里永远走else
...
###########进入update module
            ...
            result.append(SpiderEncoderState(
                ...
                question_memory=q_enc_new_item,  # [1, question_length, 768]
                ...
                pointer_maps={
                    'column': column_pointer_maps[batch_idx], #对应的col index
                    'table': table_pointer_maps[batch_idx],   #对应的table index
                },
    ...
    def pad_sequence_for_bert_batch(self, tokens_lists):
        ...
        for item_toks in tokens_lists:
            padded_item_toks = item_toks + [pad_id] * (max_len - len(item_toks))   #直接padding到max
            toks_ids.append(padded_item_toks)

            _att_mask = [1] * len(item_toks) + [0] * (max_len - len(item_toks))    #attention mask 生成
            att_masks.append(_att_mask)

            first_sep_id = padded_item_toks.index(self.tokenizer.sep_token_id)
            assert first_sep_id > 0
            _tok_type_list = [0] * (first_sep_id + 1) + [1] * (max_len - first_sep_id - 1) #将[cls]question[sep] 设置为0，其余设置为1
