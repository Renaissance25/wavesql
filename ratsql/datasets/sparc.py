...
class SpiderDataset(torch.utils.data.Dataset):
    def __init__(self, paths, tables_paths, db_path, demo_path=None, limit=None):
        ...
        for path in paths:
            ...
            for entry in raw_data:
                ...
                querys = []
                querys_toks = []
                for utterance_info in entry["interaction"]:
                    utterances_toks.append(utterance_info["utterance_toks"])
                    parse_sqls.append(utterance_info["sql"])
                    utterances_ori.append(utterance_info["utterance"])
                    querys.append(utterance_info["query"])
                    querys_toks.append(utterance_info["query_toks_no_value"])

                item = SparcItem(
                    utterances=utterances_ori,
                    utterances_toks=utterances_toks,
                    querys=querys,
                    querys_toks=querys_toks,
                    sqls=parse_sqls,
    ...
    class Metrics:
        ...
        def add(self, schema_id, ori_query, inferred_code, orig_question=None):
            ret_dict = self.evaluator.evaluate_one(
                schema_id, ori_query, inferred_code)
            if orig_question:
