...
import _jsonnet
import attr
from ratsql.commands import sparc_preprocess, sparc_train, sparc_infer, sparc_eval
...
def main():
    ...
    if args.mode == "preprocess":
        ...
        sparc_preprocess.main(preprocess_config)
    elif args.mode == "train":
        train_config = TrainConfig(model_config_file,
                                   model_config_args, logdir)
        sparc_train.main(train_config)
    elif args.mode == "eval":                   #如果是eval
        for step in exp_config["eval_steps"]:
            ...
            real_eval_path = sparc_eval.main(eval_config)

            if real_eval_path:
                res_json = json.load(open(real_eval_path))
            else:
